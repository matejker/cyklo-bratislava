...
const HeatMap = () => {
    const rentMorningValues = rent_mornings.map(v => v.reduce((partialSum, a) => partialSum + a, 0));
    const returnMorningValues = return_mornings.map(v => v.reduce((partialSum, a) => partialSum + a, 0));
    const rentEveningValues = rent_mornings.map(v => v.reduce((partialSum, a) => partialSum + a, 0));
    const returnEveningValues = return_evenings.map(v => v.reduce((partialSum, a) => partialSum + a, 0));
    ...
    return (
        <>
            ...
            <>
                <ul className={'inline'}>
                    <li><input type={'radio'} value={'rent'} id={'radio-rent'} checked={checkedValue === 'Rents'} onChange={() => setCheckedValue('Rents')} />
                        <label htmlFor={'radio-rent'}>Rents</label></li>
                    <li><input type={'radio'} value={'return'} id={'radio-return'} checked={checkedValue === 'Returns'} onChange={() => setCheckedValue('Returns')} />
                        <label htmlFor={'radio-return'}>Returns</label></li>
                {/*</ul>*/}
                {/*<ul className={'inline'}>*/}
                    <li><input type={'radio'} value={'morning'} id={'radio-morning'} checked={timeChecked === 'Mornings'} onChange={() => setTimeChecked('Mornings')} />
                        <label htmlFor={'radio-morning'}>Mornings</label></li>
                    <li><input type={'radio'} value={'afternoon'} id={'radio-afternoon'} checked={timeChecked === 'Afternoons'} onChange={() => setTimeChecked('Afternoons')} />
                        <label htmlFor={'radio-afternoon'}>Afternoons</label></li>
                </ul>
            </>
